...
class Settings(BaseSettings):
    ...
    cors_origins: str = (
        ...
        "http://localhost:3001,http://127.0.0.1:3001,http://192.168.29.43:3001,"
    ...
    # --- orca-core additions --------------------------------------------------
    # This service is the merge of the ORCA backend and the HackHeritage stack.
    # It runs on its own port so both originals keep working untouched while it
    # is built; nothing here reaches into either of them.
    service_name: str = "orca-core"
    port: int = 8100

    # The archive. ORCA's original backend keeps nothing between days: caches are
    # in-memory dicts keyed by date and yesterday's entries are deleted on the
    # next call, so every bulletin parsed and every advisory scraped is lost
    # within 24 hours. That is what blocks learning a PFZ gap-filler or asking
    # "what did IMD say last week". A file-backed archive turns the scrapers
    # that already run into dataset generators, at no extra fetching.
    archive_path: str = "data/archive.sqlite3"
    archive_enabled: bool = True

    # The evidence corpus behind retrieval. Real bulletins and notifications
    # only — the corpus this replaces was fourteen written summaries carrying
    # plausible-looking identifiers, which is worse than no corpus because a
    # fabricated closure date reads exactly like a real one.
    evidence_corpus_path: str = "data/evidence/corpus.json"
